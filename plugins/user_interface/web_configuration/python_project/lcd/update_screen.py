                ...
                if self.command_router.data['status'] == 'play':
                    ...
                    if currentTime - self.lastUpdate > 0:
                        ...
                        if self.offset_state == 0:
                            ...
                            if tft.MAX_CHARS_PER_LINE + self.offset_title < len(self.command_router.data['title']):
                                self.offset_title += 1
                            else:
                                self.offset_title = 0
                                self.offset_state = 2
                        elif self.offset_state == 2:
                            if tft.MAX_CHARS_PER_LINE + self.offset_artist < len(self.command_router.data['artist']):
                                self.offset_artist += 1
                            else:
                                self.offset_artist = 0
                                self.offset_state = 3
                        elif self.offset_state == 3:
                            if tft.MAX_CHARS_PER_LINE + self.offset_album < len(self.command_router.data['album']):
                                self.offset_album += 1
                            else:
                                self.offset_album = 0
                                self.offset_state = 0
